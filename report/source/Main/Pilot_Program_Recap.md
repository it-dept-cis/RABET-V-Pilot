...
During the pilot, it became apparent we needed a concise way to define and organize the RABET-V efforts beyond the process activities defined in the framework. We decided to define ten Security Control Families (sometimes referred to by security services) that are used throughout RABET-V to help evaluate the product. These are used to organize the requirements, architecture evaluation, verification testing, and results. These are defined in the Program Description.
...
RABET-V attempts to address these various needs by 1. Organizing the results into three maturity indexes which reflect the main conclusions drawn from the RABET-V process, and 2. Providing a single verification decision which can be used or ignored by a state election jurisdiction. 

The three maturity indexes reflect the results from the process assessment, architecture review, and the verification activities of the RABET-V process.
...
The SSCM index identifies the product's current capability level measured across ten security control families. 
...
Throughout the pilot, we discussed the question of the visibility of the testing results. There are many competing approaches to this and it is something that should be further investigated in future RABET-V efforts. Our proposal defines four audiences - public, subscribers, technology providers, and partners - and suggests the following visibility rules related to RABET-V artifacts.
...
- The name and version number of each product and product revision that has completing or is currently in the RABET-V process
...
- Top level maturity index average for each product revision that has completed the RABET-V process

Subscribers - these are state and local election jurisdictions

- Test matrix and testing methods used for each product version
- Mid-level scores for maturity index
...
We envision a role for partners like the Cybersecurity and Infrastructure Security Agency (CISA) and other information sharing partners. We believe the level of information may vary by partner.
...
To support our analysis, we used three software tools. We used a software architecture management tool called Lattix. We used a security and license compliance management tool called WhiteSource. Finally, we used a software modeling tool called MagicDraw. Each of these tools was beneficial in some way to the analysis.

Lattix performs analysis on the software solution using either source code or executable code and builds a dependency matrix. This illuminates how the system is constructed without having to perform source code analysis. We worked with each provider to identify the software modules related to each security control family, and then we analyzed the internal and external dependencies of each of those modules. 

Separately, Lattix provides a change analysis capability. This highlights the software modules which have been updated from the prior version and can provide detailed listing of adding, removed, and updated elements within each module. We did not use this feature in this initial pilot but believe it has great potential benefits in RABET-V. 

WhiteSource provided us additional insight into the 3rd party dependencies of the systems. Specifically, it identified the 3rd party versions used and whether those versions were reasonably updated or had known security vulnerabilities. 
...
Going forward, we have many suggestions for how to improve the efficiency of the architecture review process. In this pilot, we focused on developing a rubric and methodology that was sustainable and would produce consistent results. The next step is developing better instructions and tooling so that technology providers can prepare diagrams and perform some analysis themselves prior to coming through RABET-V. 

### Incorporating Usability and Accessibility

RABET-V is primarily intended to be a verification of product security. However, we heard early on from stakeholders that it needed to also provide a measure of the usability and accessibility. We attempted to solve for this in the initial Program Description by expanding the process assessment to include usability and accessibility process maturity. The assessment focuses on establishing feedback loops as the basic level of maturity and growing to more automated and more formal testing. Each provider in the pilot was evaluated with this assessment. 
