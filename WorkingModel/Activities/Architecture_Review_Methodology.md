...
The RABET-V Architecture Review is designed to evaluate the solution's architectural support for the 10 [RABET-V security services](../MaturityIndexes/Security_Services.md). This evaluation produces an architectural maturity score for each security service and identifies the components which provide the security service. This score does not measure how well the product achieves the security service (i.e. its capability level), just how mature the architecture is that supports the current capability level. The [Security Services Capability Maturity](../MaturityIndexes/Security_Services_Architectural_Maturity_Index.md) level is a separate metric determined in the [Security Claims Review](./Security_Claims_Validation.md) and it indicates how well the product provides the security services.
